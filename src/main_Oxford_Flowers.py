...
if __name__=='__main__':
    ...
    augmented_train_dataset, train_dataset, val_or_test_dataset = utils.get_oxford_flowers_datasets(root=args.dataset_path, n=args.n, tune=args.tune, random_state=args.random_state)
    # PyTorch DataLoaders
    ...
    for epoch in range(epochs):
        train_metrics = utils.train_one_epoch(model, criterion, optimizer, augmented_train_loader, scheduler, num_classes=num_heads)
        #train_metrics = utils.evaluate(model, criterion, train_loader)
        
        if args.tune or epoch == epochs-1:
            val_or_test_metrics = utils.evaluate(model, criterion, val_or_test_loader, num_classes=num_heads)
        else:
