...
class ImageDataset2D(Dataset):
    ...
    def __init__(self, df, transform_images=True, mean_and_std=None):
        ...
        self.tranform = torchvision.transforms.Compose([torchvision.transforms.Lambda(lambda x: x/255), torchvision.transforms.Resize((224, 224)), torchvision.transforms.Normalize(mean=self.mean_and_std[0], std=self.mean_and_std[1])])
...
def evaluate(model, prior_params, criterion, dataloader):
    ...
    device = torch.device('cuda:0' if next(model.parameters()).is_cuda else 'cpu')
    auc = torchmetrics.AUROC(task='multiclass', num_classes=model.fc.out_features, average='macro')
